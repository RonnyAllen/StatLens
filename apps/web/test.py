...
def run():
    ...
    columns = [g["id"] for g in sheet.get("columnGroups", [])]
    col_id_to_name = {g["id"]: g["name"] for g in sheet.get("columnGroups", [])}
    ...
    # We must pad to make it a dataframe if unequal lengths
    ...
    try:
        ...
        if table_type == "Column":
            ...
            if test_id == "Chi-Square Test" or test_id == "Chi-Square Test (with Yates' correction)":
                df_clean = df.dropna(how='all', axis=1).dropna(how='all', axis=0).fillna(0)
            ...
            if len(columns) >= 2:
                ...
                if test_id == "Two-way ANOVA":
                    ...
                    try:
                        res = pg.anova(data=df_long, dv='Value', between=[factor1_col, 'Factor2'])
            ...
            if len(columns) >= 2:
                ...
                if test_id == "Kaplan-Meier Survival Analysis":
                    if len(T) == 0:
                        result["error"] = "Insufficient valid time and event data for Kaplan-Meier."
                    else:
                        kmf = KaplanMeierFitter()
                        kmf.fit(T, event_observed=E)
                        
                        report = f"A **Kaplan-Meier Survival Analysis** was performed.\\n\\n"
                        report += f"**Median Survival Time:** {kmf.median_survival_time_}\\n\\n"
                        
                        report += "| Time | Survival Probability | 95% CI Lower | 95% CI Upper |\\n"
                        report += "|---|---|---|---|\\n"
                        ...
                        for t in st.index:
                            prob = st.loc[t].values[0]
                            lower = ci.loc[t].values[0]
                            upper = ci.loc[t].values[1]
                            report += f"| {t:.2f} | {prob:.3f} | {lower:.3f} | {upper:.3f} |\\n"
                            
                        result["report_markdown"] = report
                    
                elif test_id == "Log-rank (Mantel-Cox) test" or test_id == "Hazard Ratios":
                    if len(columns) >= 3:
                        group_col = columns[2]
                        ...
                        if len(df_clean) == 0 or len(np.unique(groups)) < 2:
                            result["error"] = "Log-rank test requires at least 2 distinct groups with valid data."
                        else:
                            res = multivariate_logrank_test(T, groups, E)
                            
                            report = f"A **Log-rank (Mantel-Cox) test** was performed comparing survival curves across groups in '{col_id_to_name.get(group_col, group_col)}'.\\n\\n"
                            sig = "significant" if res.p_value < 0.05 else "not significant"
                            report += f"The difference between survival curves is **{sig}** (Chi-square = {res.test_statistic:.3f}, p = {res.p_value:.4f}).\\n\\n"
                            
                            report += "### Summary of Data\\n"
                            report += "| Group | N | Events |\\n|---|---|---|\\n"
                            for g in np.unique(groups):
                                g_mask = groups == g
                                report += f"| {g} | {g_mask.sum()} | {E[g_mask].sum()} |\\n"
                            
                            if test_id == "Hazard Ratios" and len(np.unique(groups)) == 2:
                                cph = CoxPHFitter()
                                cph.fit(df_clean[[time_col, event_col, group_col]], duration_col=time_col, event_col=event_col)
                                hr = cph.hazard_ratios_.values[0]
                                report += f"\\n**Hazard Ratio:** {hr:.3f} (for {col_id_to_name.get(group_col, group_col)})"
                                
                            result["report_markdown"] = report
                    else:
                        # Fallback for 1 group vs expectation? Log rank needs 2 groups.
                        result["error"] = "Log-rank test requires at least 3 columns: Time, Event, and Group."
