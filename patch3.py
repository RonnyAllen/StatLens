...
target_str4 = '                result["report_markdown"] = report'
# Note: target_str4 appears in a few places! Let's be MORE specific for Partial Correlation.
...
target_str5 = '                        result["report_markdown"] = report\n            else:\n                result["error"] = "Survival Analysis requires Time and at least one Group column."'

if 'elif test_id == "Log-rank Trend Test":' not in content:
    content = content.replace(target_str5, target_str5 + '\n' + logrank)

# 5. Fix Fisher-Freeman-Halton timeout in `niche_stats.test.ts`? No, wait, I also want to make sure I update the menus again because I checked out the file!
# Let's add the menu update logic.
